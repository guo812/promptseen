...
export default async function AccountPage() {
  ...
  const cards = [
    ...
    ['Billing', 'Stripe or Creem checkout references and plan status are shown only to the signed-in user.'],
  ];
  return (
    <PageShell>
      <section className="section wrap page-hero">
        <SectionHeader as="h1" eyebrow="Account dashboard" title="Credits, saved prompts, history, and billing states">
          {session ? `Signed in as ${session.email}. Your server-rendered account page updates from the secure session cookie.` : 'Logged-out users see a clear sign-in requirement instead of fake dashboard data.'}
        </SectionHeader>
        ...
        <div className="hero-actions">
          {session ? <Link className="btn btn-primary" href="/generate">Generate an image</Link> : <Link className="btn btn-primary" href="/app/sign-in">Sign in</Link>}
